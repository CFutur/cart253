...
*/
function draw() {
    // Make the background light blue (specified as RGB)
    ...
    for (let level = levels; level > 0; level--) {
        ...
        // (500, 500) with a size based on the level
        // e.g. level 1 will be a 48x48 rectangle and
        // level 10 will be a 480x480 rectangle
        rect(500, 500, level * 100, level * 100);
        pop();
    }

    // Draw a yellow circle at the position of the mouse
    push();
    // No line around the shape
    noStroke();
    // Make it yellow (RGB)
    fill(254,255,186);
    // Draw a 10x10 circle at the mouse position
